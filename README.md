    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        .tb-btn.tb-reset {
            ...
            color: var(--rose);
            border: 1px solid rgba(201, 126, 138, 0.2);
        }
        ...
        .comment-avatar {
            width: 24px; height: 24px; border-radius: 6px; background: var(--glass-hover); color: var(--gold); font-size: 0.7 hollow; font-weight: 700; display: flex; align-items: center; justify-content: center; overflow: hidden;
        }
...
<body>
    ...
    <div class="toolbar">
        ...
        <button class="tb-btn tb-reset" onclick="resetApplicationState()">↺ Reset App</button>
    </div>
    ...
    <script>
        ...
        /* ── REGISTRATION BACKFLOW (APPLICATION STATE PRESERVED) ── */
        function resetApplicationState() {
            // Log out current session context only
            S.isRegistered = false;
            S.user = { name: 'Guest', email: '', avatar: null };
            
            // Sync with memory state without destroying neighbor array data
            saveStateToStorage();
            
            // Clear entry field states explicitly
            document.getElementById('reg-name').value = '';
            document.getElementById('reg-email').value = '';
            document.getElementById('reg-pass').value = '';
            
            toast('Returned to registration gate. Community members preserved.', 'gold');
            go('gate');
        }
        ...
        function toast(msg, type = 'gold') {
            ...
            el.textContent = msg;
            el.style.borderLeftColor = type === 'sage' ? 'var(--sage)' : (type === 'rose' ? 'var(--rose)' : 'var(--gold)');
            el.style.display = 'block';
            clearTimeout(toastTimer);
            toastTimer = setTimeout(() => { el.style.display = 'none'; }, 2800);
        }

        /* ── SIGNUP HANDLING WITH CAP ── */
        function handleSignup() {
            ...
            const existingMember = S.members.find(m => m.email === e);

            // Enforce registration cap only if it is a new member signing up
            if (!existingMember && S.members.length >= 1000) {
                toast('Registration failed. The community has reached its capacity limit of 1,000 members.', 'rose');
                return;
            }

            S.user = { name: n, email: e, avatar: existingMember ? existingMember.avatar : null };
            S.isRegistered = true; 
        ...
        function renderMembers() {
            ...
            document.getElementById('members-eyebrow').textContent = `Registered · ${S.members.length} / 1000`;
            el.innerHTML = '';
